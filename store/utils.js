...
function makeAction({ baseUrl = '', method = 'GET', mutationType, url, defaultQuery }, resolve, reject) {
	return ({ commit }, { query } = {}) => {
		const data = { ...defaultQuery, ...query};
		console.log('requestBaseUrl[baseUrl].baseUrl',requestBaseUrl[baseUrl],requestBaseUrl[baseUrl].baseUrl);
		return uniRequest({
			method,
